...
in
{
  ...
  testScript = ''
    ...
    with subtest("Ephemeral"):
        ...
        wait_until_stopped(server, "systemd-nspawn@ephemeral")
    ...
    wait_until_stopped(server, "systemd-nspawn@container0")
    wait_until_stopped(server, "systemd-nspawn@container1")
