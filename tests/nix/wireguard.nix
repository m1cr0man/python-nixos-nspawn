{ pkgs, lib, ... }:
let
  keypairs = { inherit (import "${pkgs.path}/nixos/tests/wireguard/snakeoil-keys.nix") peer0 peer1; };
    ...
    { config, pkgs, ... }:
    {
      ...
      systemd.network = {
        netdevs."20-wg0" = {
          ...
          wireguardConfig = {
            ListenPort = 23542;
            PrivateKeyFile = pkgs.writeText "wg0-priv" keypairs.${config.networking.hostName}.privateKey;
          };
        };
        networks."20-wg0" = {
        ...
        # or systemd.network(5) docs on why this is required
...
in
{
    ...
    peer0 =
      { pkgs, ... }:
      {
        ...
        systemd.network = {
          ...
          networks."20-wg0".address = [
            "192.168.20.1/22"
            "fd42:23:dead:42::1/56"
          ];
        };
      };
    peer1 =
      { pkgs, ... }:
      {
        ...
        nixos.containers.zones.zone0 = {
          ...
          ipv6Prefixes = [ { Prefix = "fd42:23:dead:43::/64"; } ];
        };

        nixos.containers.instances = {
          container0.zone = "zone0";
        };
  ...
  testScript = ''
    ...
    with subtest("WireGuard tunnel is up"):
        peer1.wait_until_succeeds("ping -c1 192.168.1.1 >&2")
        peer0.wait_until_succeeds("ping -c1 192.168.1.2 >&2")

        peer1.wait_until_succeeds("ping -c1 192.168.20.1 >&2")
        peer0.wait_until_succeeds("ping -c1 192.168.20.2 >&2")
        peer1.wait_until_succeeds("ping -c1 fd42:23:dead:42::1 >&2")
        peer0.wait_until_succeeds("ping -c1 fd42:23:dead:42::2 >&2")

    with subtest("IPv4 connectivty to container"):
        peer1.wait_until_succeeds("ping -4 -c1 container0 >&2")
        peer0.succeed(f"ping -c1 {get_container_ip('container0', 4)} >&2")

    with subtest("IPv6 connectivity to container"):
        peer1.wait_until_succeeds("ping -6 -c1 container0 >&2")
        peer0.succeed(f"ping -6 -c1 {get_container_ip('container0', 6)} >&2")

    with subtest("DNS"):
        for ip_version in [4, 6]:
            peer1.wait_until_succeeds(f"ping -c1 -{ip_version} >&2 peer0.vpn")
            peer0.wait_until_succeeds(f"ping -c1 -{ip_version} >&2 peer1.vpn")

            peer1.succeed(f"ping -c1 -{ip_version} >&2 container0.zone0.peer1.vpn")
            peer0.succeed(f"ping -c1 -{ip_version} >&2 container0.zone0.peer1.vpn")

        # other records don't give NXDOMAIN
        peer1.succeed("${pkgs.bind.dnsutils}/bin/dig @192.168.20.2 -t MX container0.zone0.peer1.vpn | tee /dev/stderr | grep -v NXDOMAIN")

        # tries to recurse it via other nameservers which times out in the sandbox
        peer1.fail("${pkgs.bind.dnsutils}/bin/dig @192.168.20.2 container42.zone0.peer1.vpn | tee /dev/stderr")
        peer1.succeed("networkctl down vb-container0")
        peer1.wait_until_fails("ping -c4 container0")
        peer1.succeed("${pkgs.bind.dnsutils}/bin/dig @192.168.20.2 container0.zone0.peer1.vpn | tee /dev/stderr | grep NOERROR")
